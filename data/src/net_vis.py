...
# https://www.cs.utah.edu/~yos/2021/02/02/plotly-python.html

# fail to add egde labels
# fail to change edge opacity
...
class GraphVisualization:
    def __init__(
        ...
        edge_opacity: Union[Dict[Edge, Num], Callable, Num] = None,
        # edge_text: Union[Dict[Vertex, str], Callable] = None,
    ):
        # check dimensions
        ...
        self.default_settings = dict(
            ...
            node_text_font_family='Georgia',
            node_text_font_size=14,
            node_size=10 if self.is_3d else 5,
            node_color='#fcfcfc',
            node_border_width=0.2,
            node_border_color='#8f8f8f', # '#6e6e6e',
            node_opacity=0.7,
            
            # can we show edge labels?

            # edge_text='bhjbkj',
            edge_width=4 if self.is_3d else 0.5,
            edge_color='#a8a8a8',
            edge_opacity=0.3,
        )
    ...
    def _get_edge_traces(self, showlabel) -> List[Union[go.Scatter, go.Scatter3d]]:
        ...
        for (color, width), edges in groups.items():
            ...
            params = dict(
                x=x,
                y=y,
                mode='lines' + ('+text' if showlabel else ''),
                hoverinfo='text',
                line=dict(color=color, width=width),
                # text=self._get_setting('edge_text'),
                # textfont=dict(
    ...
    def create_figure(
        ...
        **params
        ...
        fig.add_traces(self._get_edge_traces(showlabel))
...
def vis_graph(G, highlight_node_color_dict, highlight_edge_color_dict):

    # G = nx.erdos_renyi_graph(300, 0.008, seed=12345)
    pos = nx.circular_layout(G)
    # more dense graph, make outliers more to the center
    # for k, v in pos.items():
    #     pos[k] = [v[0] * 0.1, v[1] * 0.1]

    # make the center of the graph more spread out
    # for k, v in pos.items():
    #     pos[k] = [v[0] * 1.5, v[1] * 1.5]

    # make edge_width based on the weights of edges
    # get the largest weight
    try:
        max_weight = max([G[u][v]["weight"] for u, v in G.edges])
        edge_widths_list = [G[u][v]["weight"] / (max_weight*1) for u, v in G.edges]
        # make the dict as ('abelard', 'aristotle-logic'): 10
        edge_widths = dict(zip(G.edges, edge_widths_list))
    except:
        edge_widths = 1

    # make the node_size based on degree
    # get the largest degree
    max_degree = max([G.degree[i] for i in G.nodes])
    min_degree = min([G.degree[i] for i in G.nodes])
    # get the 75% percentile degree
    upper_quartile_degree = np.percentile([G.degree[i] for i in G.nodes], 75)

    # get the median degree
    median_degree = np.median([G.degree[i] for i in G.nodes])
    mean_degree = np.mean([G.degree[i] for i in G.nodes])
    # find the smallest between median and mean, in one line
    median_degree = min(np.median([G.degree[i] for i in G.nodes]), np.mean([G.degree[i] for i in G.nodes]))

    node_size_list = [G.degree[i] for i in G.nodes]
    # normalize the the list  (float(i)-min(a))/(max(a)-min(a))
    # node_size_list = [(float(i)-min_degree)/(max_degree-min_degree) for i in node_size_list]
    node_size_list = [(float(i) / (upper_quartile_degree) + 5) for i in node_size_list]
    node_size = dict(zip(G.nodes, node_size_list))
    ...
    vis = GraphVisualization(
        ...
        node_size = node_size,
        node_border_width=1, 
        node_color = highlight_node_color_dict,
        # node_color=int,                   # color vertices based on their IDs
        
        node_text_font_size={4: 30},
        
        edge_color= highlight_edge_color_dict,
        # edge_opacity= 0.5,
        # highlight_edge_opacity_dict,
...
def vis_graph_design(G, highlight_node_list_color, highlight_edge_list_color
                     ):

    # ============== highlight_node_color
    highlight_node_color_dict = {}
    for color in highlight_node_list_color:
        node_list = highlight_node_list_color[color]
        for node in node_list:
            highlight_node_color_dict[(node)] = color

    # ============== highlight_edge_color
    highlight_edge_color_dict = {}
    for color in highlight_edge_list_color: # find the edges that have the nodes in the list
        edge_list = highlight_edge_list_color[color]
        for edge in edge_list:
            highlight_edge_color_dict[(edge)] = color
    # highlight_edge_opacity_dict = {}
    # # higlighted edges are less transparent
    # for edge in G.edges:
    #     if edge in edge_list:
    #         highlight_edge_opacity_dict[(edge)] = 1
    #     else:
    #         highlight_edge_opacity_dict[(edge)] = 0.3


    
    # ============== highlight_node_size
    # highlight_node_size_dict = {}
    # for i in highlight_node_list:
    #     highlight_node_size_dict[(i)] = highlight_node_size

    vis_graph(G, highlight_node_color_dict, highlight_edge_color_dict)
