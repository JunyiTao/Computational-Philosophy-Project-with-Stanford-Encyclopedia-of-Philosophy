...
def vis_query(query, score_by_token):
	for token, _ in score_by_token:
	...
	for q in query:
		q = q.lower()
		matched = False
		for token, score in score_by_token:
			token = token.lower()
			if token in q:
				matched = True
		...
		if not matched:
			score_list.append(0)
	generate(query, score_list, "query.tex", color='RoyalPurple', rescale_value=True)
